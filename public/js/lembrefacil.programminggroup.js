(function($) {
    $.addProgrammingGroup = function(element, options) {
        var pg = {
            root: $(element),
            ...
            getFormValues: function() {
                ...
                row[pg.IDX_TYPE] = '2'; // sms send option SMS
                // Pega os valores
                row[pg.IDX_DESCR] = r0.val();
                row[pg.IDX_DTS] = r1.val();
                row[pg.IDX_DTE] = r2.val();
                row[pg.IDX_HR] = new Array();
                r3.find('.formElementMultiValues').each(function(i) {
                    var t = $(this).val();
                    if (t) row[pg.IDX_HR].push($(this).val());
                });
                row[pg.IDX_RMD] = r4.val();
                ...
                if (row[pg.IDX_RMD] && !intVerify(row[pg.IDX_RMD])) {
            ...
            resetFormValues: function() {
                ...
                $('#time input').val('');
                $('#reminder').val('');
                $('#in_repetition-0').attr('checked', 'checked');
                $('#frequency').val(1);
            ...
            setFormValues: function(row) {
                ...
                $('#description').val(row[pg.IDX_DESCR]);
                $('#dt_start').val(row[pg.IDX_DTS]);
                $('#dt_end').val(row[pg.IDX_DTE]);
                var i;
                for (i in row[3]) {
                ...
                $('#reminder').val(row[pg.IDX_RMD]);
                $('#in_repetition-' + row[pg.IDX_REP]).attr('checked', 'checked');
                if (row[pg.IDX_REP] == '1') {
                    $('#frequency').val(row[pg.IDX_FREQ]);
                    $('#interact-in_repetition-1').show();
                    if (row[6] != '1') $('#frequency-' + row[pg.IDX_FREQ]).show();
            ...
            editRow: function(key) {
                pg.root.find('.programmingGroupTable tbody tr:eq(' + pg.currentRow + ')').removeClass('edit');
                pg.currentRow = key;
            ...
            createRow: function(row) {
                ...
                //tr.append('<td>' + row[pg.IDX_QTY] + '</td>');
                tr.append('<td>' + row[pg.IDX_DTS] + '</td>');
                tr.append('<td>' + ((row[pg.IDX_DTE]) ? row[pg.IDX_DTE] : '') + '</td>');
                var times = '';
                    ...
                    case 1:
                        switch (parseInt(row[pg.IDX_FREQ], 10)) {
                            ...
                            case 2:
                                ...
                                for (i in row[pg.IDX_DUR]) {
                                    var day = $('label[for='+ $('input[name=week_frequency[]][value=' +  row[7][i] + ']').attr('id') + ']').text()
                        ...
                        break;
                    case 2:
                        if (row[6] === 0) repetition = 'Envio durante ' + row[pg.IDX_DUR] + ' dias com a interrupção de ' + row[8]  + ' dias';
    ...
    $.fn.programmingGroup = function(options) {
        return this.each(function() {
            $.addProgrammingGroup(this, options);
        });
    };
})(jQuery);